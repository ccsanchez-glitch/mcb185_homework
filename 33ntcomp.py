...
# variation creating list to hold the counts
#assignments are at specific indices of the list variable counts 
...
for defline, seq in mcb185.read_fasta(sys.argv[1]):
	defwords = defline.split()
	name = defwords[0]
	nts = 'ACGTN'	#string named nts 
	counts = [0] * len(nts)	#shortcut for assigning a bunch of zeros
	for nt in seq:  
		idx = nts.find(nt)
		counts[idx] += 1
	print(name, end='')
